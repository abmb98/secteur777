  ...
  const handleCreateTransfer = async () => {
    try {
      ...
      const newTransfer: Partial<StockTransfer> = {
        ...
        fromFermeName: getFermeName(user?.fermeId || ''),
      ...
      const notification: Partial<TransferNotification> = {
        ...
        quantity: parseInt(transferForm.quantity),
        unit: sourceStock.unit,
        message: `Nouveau transfert entrant: ${sourceStock.item} (${transferForm.quantity} ${sourceStock.unit}) de ${getFermeName(user?.fermeId || '')}`,
        status: 'unread',
        createdAt: serverTimestamp(),
    ...
    <div className="space-y-6">
          ...
          <Button
            onClick={() => {
              ...
              toast({
                ...
                description: "Tous les articles sont maintenant affichés"
              });
            }}
            variant="outline"
          >
            ...
            <div>
              ...
              >
                ...
                <SelectContent>
                  {stocks
                    .filter(stock => {
                      const matches = stock.secteurId === user?.fermeId;
                      console.log('Stock filter check:', {
                        stockItem: stock.item,
                        stockSecteurId: stock.secteurId,
                        userFermeId: user?.fermeId,
                        matches
                      });
                      return matches;
                  ...
                  {stocks.filter(stock => stock.secteurId === user?.fermeId).length === 0 && (
      ...
      <AlertDialog open={showConfirmDialog} onOpenChange={setShowConfirmDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>
              {confirmAction === 'confirm' ? 'Confirmer le transfert' : 'Rejeter le transfert'}
            </AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction
              ...
              className={confirmAction === 'confirm' ? 'bg-green-600 hover:bg-green-700' : 'bg-red-600 hover:bg-red-700'}
            >
              {confirmAction === 'confirm' ? 'Confirmer' : 'Rejeter'}
            </AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
